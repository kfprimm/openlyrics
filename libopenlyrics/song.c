...
OLSong *ol_song_new() { return ol_new_object(sizeof(OLSong)); }
...
{  
  ...
  song->titles[song->num_titles-1]=ol_title_new(title, lang);
}
...
void ol_song_add_author  (OLSong *song, const char *author, OL_AUTHOR_TYPE type, const char *lang)
{
  ...
  song->authors[song->num_authors-1]=ol_author_new(author,type,lang);
}
...
void ol_song_add_songbook  (OLSong *song, const char *name, const char *entry, const char *lang)
{
  ...
  song->songbooks[song->num_songbooks-1]=ol_songbook_new(name, entry, lang);
}
